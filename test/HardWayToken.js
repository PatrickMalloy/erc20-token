...
describe("HardWayToken", function () {
  ...
  async function deployHWTFixture() {
    ...
    //const HWT = await ethers.getContractFactory("HardWayToken");
    //const hwt = await HWT.deploy("HardWayToken", [initialSupply, maxSupply]);
    const hwt = await hre.ethers.deployContract("HardWayToken", [initialSupply, maxSupply]);
    await hwt.waitForDeployment();
    //console.log(`HardwayToken deployed at ${hwt.target}`);

    return { hwt, owner, account1, initialSupply, maxSupply };
  }
